...
# auto scale the lr, maximum iteration, lr step by the batch_size
# the numeric scale is 8.
...
# each gpu at least should have 6 
...
# NOTE
# ==== GAN related functions, some are decrypted ====
...
# NOTE
# ==== DistributedDataParallel related functions ====
...
def main(rank, world_size, net):
    '''
    rank (int): 
        the implict process ID (also be used as device IDs) 
        which will be assign when the multi-processes are created.
    world_size (int):
        the number of processes one wants to spawn.
    net (class):
        the model class which is going to be warped in this function
    '''
